...
; Using AT89S52 interfaced with DS1302.
;
...
DSEG
  ...
  puluhan:  ; For stopwatch, puluhan means digit for tens
  DS  1

  satuan:   ; For stopwatch, satuan means digit for ones
  DS  1
...
pass6:  ; digit ke-6 bukan '*' & '#'
  ...
  setb  AFlag   ; Alarm is set / modify time
ret
...
exitSW:
  ...
  mov   dptr, #empty
  call  tulisString
  call  baris2             ; Jump to 2nd row
  mov   dptr, #kalimat9    ; and show information
  call  tulisString
  jmp   loop
...
setWaktu:
  ...
  call  WRENB
  mov   A, alarm??+2     ; Update hour
  mov   B, #HR
  call  BYTEWRCLKREG
  mov   A, alarm??+1     ; Update minute
  mov   B, #MIN
  call  BYTEWRCLKREG
  mov   A, alarm??       ; Update sec
  mov   B, #SEC
  call  BYTEWRCLKREG
  call  WRDIS
  clr   AFlag
...
clearFlag:
  ...
  clr   AFlagLed
  clr   AFlagBuzz
gakDiteken:              ; Not pressed / '*' @1st row
  jmp   loop
...
tulisNibble:
  ...
  orl   A,   #0Fh      ; Doesn't affect bit #0-3
